# detector.py - Lightweight Braille Detection (NO inference-sdk)
import json
import base64
...
class BrailleDetector:
    ...
    def detect_braille(self, image_path: str) -> Dict:
        """Run Braille detection using direct Roboflow API call"""
        try:
            # Read image file
            ...
            response = requests.post(
                f"{self.api_url}/{self.workspace_name}/{self.workflow_id}",
    ...
    def organize_text_by_rows(self, predictions: List[Dict], min_confidence: float = 0.4) -> List[str]:
        """Organize detected characters into rows (same as before)"""
    ...
    def create_annotated_image(self, image_path: str, predictions: List[Dict], 
        ...
        """Create annotated image (same as before)"""
        # [Keep your existing implementation]
        try:
